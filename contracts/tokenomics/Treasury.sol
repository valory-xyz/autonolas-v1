...
// import "hardhat/console.sol"; // for troubleshooting
...
contract OlympusTreasury is OlympusAccessControlled, ITreasury {
    ...
    constructor(
        ...
        require(_ola != address(0), "Zero address: OLA");
        OLA = IOLA(_ola);
    ...
    /**
     * @notice allow approved address to deposit an asset for OLA
    ...
    function deposit(
        ...
        // mint OLA needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        OLA.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn OLA for reserves
     * @param _amount uint256
     * @param _token address
     */
    ...
    /**
     * @notice mint new OLA using excess reserves
     * @param _recipient address
     * @param _amount uint256
     */
    function mint(address _recipient, uint256 _amount) external override {
        require(permissions[STATUS.REWARDMANAGER][msg.sender], notApproved);
        // console.log("Treasury :: mint :: msg.sender[%s] approved",msg.sender);
        // console.log("Treasury :: totalReserves:%d OLA.totalSupply():%d",totalReserves,OLA.totalSupply());
        require(_amount <= excessReserves(), insufficientReserves);
        // console.log("Treasury :: mint[%s][%s]:%d",address(OLA),_recipient,_amount);
        OLA.mint(_recipient, _amount);
        // console.log("Treasury :: mint[%s][%s]:%d -- OK",address(OLA),_recipient,_amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * @notice mint new OLA using excess reserves. debug version
     * @param _recipient address
     * @param _amount uint256
     */
    function mintNoCheck(address _recipient, uint256 _amount) external override { // Rewrite original mint with log and pass excessReserves()
        require(permissions[STATUS.REWARDMANAGER][msg.sender], notApproved);
        // console.log("Treasury :: mintNoCheck :: msg.sender[%s] approved",msg.sender);
        bool insufficientReservesFlag = false;
        if(totalReserves >= OLA.totalSupply()) {
            insufficientReservesFlag = true;
        } 
        // console.log("Treasury :: totalReserves:%d OLA.totalSupply:%d insufficientReserves:%s",totalReserves,OLA.totalSupply(),insufficientReservesFlag);
        // require(_amount <= excessReserves(), insufficientReserves); // issue with deposit to cold treasure
        // console.log("Treasury :: mintNoCheck[%s][%s]:%d",address(OLA),_recipient,_amount);
        OLA.mint(_recipient, _amount);
        // console.log("Treasury :: mintNoCheck[%s][%s]:%d -- OK",address(OLA),_recipient,_amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * sOHM needed for implementation. original. disabled
    ...
    /**
     * @dev sOHM needed for implementation. original. disabled
    ...
    */
    ...
    function excessReserves() public view override returns (uint256) {
        // return totalReserves.sub(OHM.totalSupply().sub(totalDebt)); // original
        return totalReserves.sub(OLA.totalSupply());
    }

    /**
     * @notice returns OLA valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(OLA)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        // return OHM.totalSupply() - ohmDebt; // original
        return OLA.totalSupply(); // ohmDebt not exist
    }
}
