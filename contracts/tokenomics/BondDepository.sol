...
/// @title Valory Bond Depository V0.1
/// @author Zeus, Indigo
/// @author AL
...
contract OlympusBondDepositoryV2 is IBondDepository, NoteKeeper {
    ...
    constructor(
        ...
        IERC20 _ola,
        ITreasury _treasury
    ) NoteKeeper(_authority, _ola, _treasury) {
     ...
     * @return payout_     the amount of OLA due
        ...
        )
    {
        ...
        // console.log("deposit LOG:: input amount:%d",_amount);
        /**
         ...
         * price = quote tokens : ola (i.e. 42069 DAI : OLA)
         *
         * 1e18 = OLA decimals (9) + price decimals (9)
         */
         ...
         *
         * this is either the number of OLA that the market can sell
        ...
        // purchased, and how much OLA has been sold
        ...
        // console.log("deposit LOG:: before market.quoteToken[%s] balance treasury[%s]:%d",address(market.quoteToken),address(treasury),market.quoteToken.balanceOf(address(treasury)));
        market.quoteToken.safeTransferFrom(msg.sender, address(treasury), _amount);
        // console.log("deposit LOG:: after market.quoteToken[%s] balance treasury[%s]:%d",address(market.quoteToken),address(treasury),market.quoteToken.balanceOf(address(treasury)));

        // if max debt is breached, the market is closed
        // this a circuit breaker
        if (term.maxDebt < market.totalDebt) {
            // console.log("market.totalDebt:%d > term.maxDebt:%d. market.capacity = 0. close market",market.totalDebt,term.maxDebt);
            market.capacity = 0;
            emit CloseMarket(_id);
        } else {
            // console.log("market.totalDebt:%d < term.maxDebt:%d. call _tune",market.totalDebt,term.maxDebt);
            // if market will continue, the control variable is tuned to hit targets on time
            _tune(_id, currentTime);
        }
        // console.log("<< stop deposit");
    }
    ...
    function _decay(uint256 _id, uint48 _time) internal {
        ...
        if (adjustments[_id].active) {
            ...
            // console.log("Bond :: _decay :: adjustBy:%d, controlVariable before adjust:%d, secondsSince:%d",adjustBy, terms[_id].controlVariable, secondsSince);
            ...
            if (stillActive) {
                ...
                // console.log("Bond :: _decay :: adjustment.active:%s",adjustment.active);
            }
        }
    }
    ...
    function _tune(uint256 _id, uint48 _time) internal {
        ...
        // console.log("_tune :: _time:%d, meta.lastTune + meta.tuneInterval:%d",_time,meta.lastTune + meta.tuneInterval);
        if (_time >= meta.lastTune + meta.tuneInterval) {
            Market memory market = markets[_id];
            // console.log("_tune :: run");
            // compute seconds remaining until market will conclude
            uint256 timeRemaining = terms[_id].conclusion - _time;
            uint256 price = _marketPrice(_id);

            // standardize capacity into an base token amount
            // ola decimals (9) + price decimals (9)
             ...
             * is 10,000 OLA. max payout would be 1,000 OLA (10,000 * 1 / 10).
             */
            ...
            if (newControlVariable >= terms[_id].controlVariable) {
                // console.log("new cv:%d > old cv:%d",newControlVariable,terms[_id].controlVariable);
                terms[_id].controlVariable = newControlVariable;
            } else {
                // if decrease, control variable change will be carried out over the tune interval
                // this is because price will be lowered
                uint64 change = terms[_id].controlVariable - newControlVariable;
                // console.log("new cv:%d < old cv:%d, delta:%d",newControlVariable,terms[_id].controlVariable,change);
     ...
     * @param _market      [capacity (in OLA or quote), initial price / OLA (9 decimals), debt buffer (3 decimals)]
    ...
    function create(
         ...
         *
         * 1e18 = ola decimals (9) + initial price decimals (9)
         */
        uint64 targetDebt = uint64(_booleans[0] ? ((_market[0] * 1e18) / _market[1]) / 10**decimals : _market[0]);

        /*
         * max payout is the amount of capacity that should be utilized in a deposit
         * interval. for example, if capacity is 1,000 OLA, there are 10 days to conclusion,
         * and the preferred deposit interval is 1 day, max payout would be 100 OLA.
         */
        ...
        emit CreateMarket(id_, address(ola), address(_quoteToken), _market[1]);
    }
     ...
     * @param _id          ID of market
     * @return             price for market in OLA decimals
     *
     ...
     * @param _id          ID of market
     * @return             amount of OLA to be paid in OLA decimals
     *
     * @dev 1e18 = ola decimals (9) + market price decimals (9)
     */
     ...
     * @param _id          ID of market
     * @return             current debt for market in OLA decimals
     */
     ...
     * @param _id          ID of market
     * @return             control variable for market in OLA decimals
     */
